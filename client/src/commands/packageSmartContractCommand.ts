...
async function getFinalDirectory(packageDir: string, workspaceDir: string): Promise<string> {
    ...
    // if (!language) {
    //     message = 'Failed to determine workspace language type, please ensure your project contains a chaincode file in the correct format (**/*.js, **/.ts, **/*.go)!';
...
async function getLanguage(workspaceDir: string): Promise<string> {
    let language: string;
    let message: string;
    const jsFiles: Array<string> = await Glob('**/*.js', {
        cwd: workspaceDir, ignore: '**/node_modules/**'
    });
    const tsFiles: Array<string> = await Glob('**/*.ts', {
        cwd: workspaceDir, ignore: '**/node_modules/**'
    });
    const goFiles: Array<string> = await Glob('**/*.go', {
        cwd: workspaceDir, ignore: '**/node_modules/**'
    });
    ...
    if (jsFiles.length > 0 && tsFiles.length > 0) {
        ...
        message = 'Please ensure you have compiled your typescript files into javascript.';
        vscode.window.showErrorMessage(message);
        throw new Error(message);
